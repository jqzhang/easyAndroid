package com.brothers.base.ui.countdown;
...
import com.brothers.base.utils.DeviceUtils;
...
class BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeTextSize = ta.getDimension(R.styleable.CountdownView_timeTextSize, DeviceUtils.sp2px(mContext, 12));
        ...
        mSuffixTextSize = ta.getDimension(R.styleable.CountdownView_suffixTextSize, DeviceUtils.sp2px(mContext, 12));
...
//        if (!isShowMinute && !isShowSecond) isShowSecond = true;
    ...
    private void initSuffixMargin() {
        int defSuffixLRMargin = DeviceUtils.dp2px(mContext, DEFAULT_SUFFIX_LR_MARGIN);
    ...
    private float getDayAndHourContentWidth() {
        ...
        if (isShowDay) {
            String tempDay = StringUtils.formatNum(mDay);
        ...
        if (isShowHour) {
            String tempHour = StringUtils.formatNum(mHour);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            // draw day text
            canvas.drawText(StringUtils.formatNum(mDay), mLeftPaddingSize + mDayTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowHour) {
            ...
            canvas.drawText(StringUtils.formatNum(mHour), mHourLeft + curTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowMinute) {
            // draw minute text
            canvas.drawText(StringUtils.formatNum(mMinute), mMinuteLeft + mTimeTextWidth / 2 , mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowSecond) {
            // draw second text
            canvas.drawText(StringUtils.formatNum(mSecond), mSecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
            ...
            if (isShowMillisecond) {
                ...
                canvas.drawText(StringUtils.formatMillisecond(mMillisecond), mMillisecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
    ...
    public void setTimeTextSize(float textSize) {
        if (textSize > 0) {
            mTimeTextSize = DeviceUtils.sp2px(mContext, textSize);
            mTimeTextPaint.setTextSize(mTimeTextSize);
        }
    }
    ...
    public void setSuffixTextSize(float textSize) {
        if (textSize > 0) {
            mSuffixTextSize = DeviceUtils.sp2px(mContext, textSize);
            mSuffixTextPaint.setTextSize(mSuffixTextSize);
        }
    }
    ...
    public void setSuffixLRMargin(float suffixLRMargin) {
        mSuffixLRMargin = DeviceUtils.dp2px(mContext, suffixLRMargin);
    ...
    public boolean setSuffixMargin(Float suffixDayMarginL, Float suffixDayMarginR,
        ...
        if (null != suffixDayMarginL) {
            mSuffixDayLeftMargin = DeviceUtils.dp2px(mContext, suffixDayMarginL);
            isRef = true;
        }
        if (null != suffixDayMarginR) {
            mSuffixDayRightMargin = DeviceUtils.dp2px(mContext, suffixDayMarginR);
            isRef = true;
        }

        if (null != suffixHourMarginL) {
            mSuffixHourLeftMargin = DeviceUtils.dp2px(mContext, suffixHourMarginL);
            isRef = true;
        }
        if (null != suffixHourMarginR) {
            mSuffixHourRightMargin = DeviceUtils.dp2px(mContext, suffixHourMarginR);
            isRef = true;
        }

        if (null != suffixMinuteMarginL) {
            mSuffixMinuteLeftMargin = DeviceUtils.dp2px(mContext, suffixMinuteMarginL);
            isRef = true;
        }
        if (null != suffixMinuteMarginR) {
            mSuffixMinuteRightMargin = DeviceUtils.dp2px(mContext, suffixMinuteMarginR);
            isRef = true;
        }

        if (null != suffixSecondMarginL) {
            mSuffixSecondLeftMargin = DeviceUtils.dp2px(mContext, suffixSecondMarginL);
            isRef = true;
        }
        if (null != suffixSecondMarginR) {
            mSuffixSecondRightMargin = DeviceUtils.dp2px(mContext, suffixSecondMarginR);
            isRef = true;
        }

        if (null != suffixMillisecondMarginL) {
            mSuffixMillisecondLeftMargin = DeviceUtils.dp2px(mContext, suffixMillisecondMarginL);
            isRef = true;
        }
